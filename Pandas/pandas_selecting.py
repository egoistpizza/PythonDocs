...
result = df.iloc[2]       # .iloc methodunda default index'lerle işlem yapabiliriz
result = df.loc[:,"Column1"]
result = df.loc[:,["Column1","Column2"]]
result = df.loc[:,"Column1":"Column3"]   # Başlangıç-Bitiş Dahil !
result = df.loc[:,"Column1":"Column2"]
...
print(df.drop("Column5", axis=1))      # .drop() metodu varsayılan olarak (inplace= False) kalıcı değişiklik yapmaz
df.drop("Column5", axis=1, inplace= True)   # inplace parametresi True verilirse kalıcı değişiklik yapar
